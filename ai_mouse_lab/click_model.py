...
def _sample_from_trial(trial: dict[str, Any]) -> dict[str, float] | None:
    ...
    try:
        ...
        dx = float(click["x"]) - float(target["x"])
        dy = float(click["y"]) - float(target["y"])
...
def build_click_model(trials: list[dict[str, Any]], max_samples: int = 1200) -> dict[str, Any]:
    ...
    bands: dict[str, list[dict[str, float]]] = {"small": [], "medium": [], "large": []}
    ...
    def summarize(group: list[dict[str, float]]) -> dict[str, Any]:
        return {
            ...
            "edge_padding_ratio": stats([item["edge_padding_ratio"] for item in group]),
            "near_edge_rate": round(sum(item["radial_ratio"] >= 0.72 for item in group) / max(1, len(group)), 4),
            "outside_rate": round(sum(item["radial_ratio"] > 1.0 for item in group) / max(1, len(group)), 4),
            "samples": [
                {
    ...
    return {
        ...
        "by_target_size": {name: summarize(group) for name, group in bands.items()},
    }
...
def sample_click_offset(
    ...
    *,
    ...
    by_size = click_model.get("by_target_size", {}) if isinstance(click_model, dict) else {}
    ...
    source = selected if int(selected.get("sample_count", 0) or 0) >= 8 else overall
    ...
    if isinstance(samples, list) and samples:
        ...
        x_ratio = rng.gauss(float(x_stats.get("mean", 0.0) or 0.0), max(0.08, float(x_stats.get("stdev", 0.22) or 0.22)))
        y_ratio = rng.gauss(float(y_stats.get("mean", 0.0) or 0.0), max(0.08, float(y_stats.get("stdev", 0.22) or 0.22)))
