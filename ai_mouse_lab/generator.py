...
def _template_shape(template: dict[str, Any] | None) -> list[dict[str, float]]:
    ...
    for point in raw_points:
        ...
        try:
            ...
            continue
    shape.sort(key=lambda point: point["t"])
    if len(shape) < 2:
        return []
    if shape[0]["t"] > 0.0:
        shape.insert(0, {"t": 0.0, "along": 0.0, "side": 0.0})
    else:
        shape[0] = {"t": 0.0, "along": 0.0, "side": 0.0}
    if shape[-1]["t"] < 1.0:
...
def _interpolate_shape(
    ...
    for first, second in zip(shape, shape[1:]):
        ...
        return (
            first["along"]
            + (second["along"] - first["along"]) * fraction,
...
def _append_profiled_route(
    ...
    *,
    ...
    px, py = -uy, ux
    steps = max(28, min(140, int(duration_ms / 8.5)))
    shape = _template_shape(template)

    if shape:
        side_variation = rng.uniform(0.94, 1.06)
        along_variation = rng.uniform(0.985, 1.015)
        for index in range(1, steps + 1):
            u = index / steps
            along_ratio, side_ratio = _interpolate_shape(shape, u)
            along = along_ratio * distance * along_variation
            side = side_ratio * distance * side_variation
            jitter = rng.gauss(0.0, 0.18 * math.sin(math.pi * u))
            t_ms = start_t_ms + duration_ms * u
            points.append(
                {
                    "t_ms": round(t_ms, 3),
    ...
    template_efficiency = (
        float(template.get("path_efficiency", 0.9))
        if isinstance(template, dict)
        else 0.9
    )
    side_scale = min(
        0.075,
        max(0.008, (1.0 - min(0.995, template_efficiency)) * 0.35),
    )
    max_side = max(
        3.0,
        min(distance * 0.10, 58.0 if distance > 350 else 24.0),
    )
    bend = max(
        -max_side,
        min(max_side, rng.gauss(0.0, distance * side_scale)),
    )
...
def generate_trial(
    ...
    movement_total_median = max(
        120.0,
        _feature(profile, "movement_time_ms", "median", 650.0),
    )
    movement_total_stdev = max(
        35.0,
        _feature(profile, "movement_time_ms", "stdev", 140.0),
    )
    reaction_median = max(
        8.0,
        _feature(profile, "reaction_ms", "median", 75.0),
    )
    reaction_stdev = max(
        8.0,
        _feature(profile, "reaction_ms", "stdev", 28.0),
    )
    click_delay_median = max(
        10.0,
        _feature(profile, "click_delay_ms", "median", 80.0),
    )
    click_delay_stdev = max(
        8.0,
        _feature(profile, "click_delay_ms", "stdev", 30.0),
    )
    hold_median = max(35.0, _feature(profile, "hold_ms", "median", 95.0))
    hold_stdev = max(6.0, _feature(profile, "hold_ms", "stdev", 18.0))
    overshoot_median = max(
        0.0,
        _feature(profile, "overshoot_px", "median", 0.0),
    )
    correction_mean = max(
        0.0,
        _feature(profile, "correction_count", "mean", 0.0),
    )
    ...
    correction = (
        not shape
        and (
            rng.random() < min(0.50, max(0.04, correction_mean / 2.0))
            or overshoot
        )
    )
    ...
    if will_miss:
        miss_down_ms = t_ms + max(8.0, click_delay_ms * 0.55)
        ...
        click_down_ms = t_ms + max(8.0, click_delay_ms * 0.45)
    else:
...
def simulate(
    ...
    rng = random.Random(
        seed if seed is not None else int(plan.get("seed", 1)) + 1
    )
    ...
    return [
        generate_trial(item, profile, rng, session_scale)
        for item in targets
    ]
